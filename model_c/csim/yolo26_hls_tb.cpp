// yolo26_hls_tb.cpp - the stage-1 bit-exact gate for the synthesizable kernel.
//
...
//
// This file is ALSO the Vitis HLS csim and cosim testbench (run_hls_zu9eg.tcl adds it with
...
int main(int argc, char** argv) {
    ...
    // ---- port buffers, sized to the DECLARED m_axi depths (yolo26_hls.h: Y26_DEPTH_*) ----
    //
    ...
    for (const auto& kv : Wg.conv) {
        ...
        // so a cosim run can shrink them (see yolo26_hls.h). Under-sizing them silently overruns the
        ...
        // instead, and name the bound that was hit.
        {
            ...
            const long need_y = (long)golden.size();
        ...
        // lane 0 = element 0, which is what the store loop in yolo26_hls.cpp writes.
#if Y26_YPE > 1
        ...
        for (size_t i = 0; i < n; ++i) {
            const double o = (double)out[i], gd = (double)golden.d[i];
            dot += o * gd; na += o * o; nb += gd * gd;
            if (memcmp(&out[i], &golden.d[i], sizeof(float)) != 0) {
                ++bad;
