...
EPS = 1e-3        # ultralytics BatchNorm2d eps (for the FP32-fallback one2many head convs)
...
class IR:
    ...
    def __init__(self, xmlpath, binpath):
        ...
        self.src = {}   # (to_id, to_port) -> (from_id, from_port)
        self.dst = {}   # from_id -> [(to_id, to_port), ...]
...
def parse_conv(ir, cid):
    ...
    sw = ir.const(ir.src[(wmul, "1")][0]).astype(np.float32).reshape(-1)      # [OC]
    if grouped:                                    # [G, OCg, ICg, KH, KW] -> [OC, ICg, KH, KW]
    ...
    if ir.T[a_id] == "FakeQuantize":
        ...
        if ir.T[f0] == "Multiply" and "nncf_smooth_quant" in ir.NM[f0]:
            ssc = ir.const(ir.src[(f0, "1")][0]).astype(np.float32).reshape(-1)   # [IC]
        has_fq = True
    else:                                          # e.g. attn.pe: input is a Reshape, not requantized
        lo = hi = 0.0
        ssc = None
        has_fq = False
...
def make_sq_forward(mod, w_int, sw, ssc, step, lo, bias):
    ...
    def fwd(x):
        xs = x * smt                                             # per-input-channel smooth pre-scale
        q = torch.clamp(torch.round((xs - lo_q) / step_q), 0.0, 255.0)   # asymmetric uint8 codes
        acc_q = F.conv2d(q.double(), w_d, None, stride, padding, dilation, groups)      # Σ w_int*q
        ones = torch.ones_like(q, dtype=torch.float64)
        acc_w = F.conv2d(ones, w_d, None, stride, padding, dilation, groups)            # Σ_valid w_int
...
def ingest(ckpt=CKPT):
    ...
    for name, mod in all_convs:
        if name not in params:                                  # one2many head: keep .pt BN-folded FP32
        ...
        if p["has_fq"]:                                         # SmoothQuant asymmetric integer path
            ...
            mod.forward = make_sq_forward(mod, w_int, sw, ssc, step, lo, bias)  # bias = ORIGINAL bias
            ...
            n_asym += 1
        else:                                                  # attn.pe: FP32 conv w/ dequant weights
...
def main():
    ...
    with open(os.path.join(WSQ, "manifest_sq.txt"), "w") as f:
        f.writelines(lines)
    # attn_fq.txt: one line per attention block -- "<block> lo hi" x5 in ATTN_KEYS order.
