...
class Compactor:
    ...
    def conv(self, name, x):
        ...
        if name in self.folded:
            ...
            for o in np.flatnonzero(m):                        # revive the matching input channels
                for (c, oo) in x[o]:
    ...
    def psablock(self, p, x):
        ...
        attn = self.conv(p + ".attn.proj.conv", x)  # proj output == x width (full)
        self.apply_add_union(x, attn)
        x2 = self.add(x, attn)
        f = self.conv(p + ".ffn.0.conv", x2)        # ffn.0 IS prunable (its 512-ch output compacts)
        f = self.conv(p + ".ffn.1.conv", f)
        self.apply_add_union(x2, f)                 # x2 is full -> forces ffn.1 output full
    ...
    def sppf(self, i, x):
        ...
        cc = self.concat([y0, y0, y0, y0])          # 3 maxpools preserve channels
        y = self.conv(s + ".cv2.conv", cc)
        if len(y) == len(x):                        # SPPF shortcut (self.add)
    ...
    def run(self):
        ...
        x12 = self.concat([x10, x6])                # upsample preserves channels
...
def _fold_dw_constants(wdir, mn, row_by, comp, keep_out):
    ...
    for dw, pw in Compactor.FOLD_PAIRS.items():
        ...
        # the constant each dropped dw channel emits (its conv response is identically 0)
        ...
        if mn == "manifest_sq.txt" and int(rp[10]) == 1:
            ...
            # FP32 (or the SmoothQuant one2many FP32-fallback convs): real-valued, scale then bias
            contrib = (w * C).sum(axis=1)
            if int(rp[10]) != 0 and mn == "manifest.txt":           # has_bn -> scale applies first
...
def main():
    ...
    # finalize keep_in / keep_out per conv
    ...
    for n in names:
        ...
        if groups == 1:
            wc = w[ko][:, ki, :, :]
            new_ic = int(ki.sum())
        else:                                        # depthwise: groups==oc==ic, keep_out==keep_in
        ...
        for suf in ("b", "sw", "s"):                 # per-OC vectors sliced by keep_out
    ...
    for i, c in comp.splits.items():
        cv1 = f"{i}.cv1.conv"
        ko = keep_out[cv1]
        comp_splits[i] = int(ko[:c].sum())           # kept channels in the first (a) half
...
def _write_manifest(wdir, outdir, mn, rows, cmap):
    ...
    for r in rows:
        ...
        r[1] = str(c["oc"])
        r[2] = str(c["ic"])
        if int(r[9]) > 1:                      # depthwise stays depthwise: groups tracks the new width
            r[9] = str(c["oc"])
        out.append(" ".join(r))
...
def _verify_lossless(wdir, outdir, mn, rows, keep_in, keep_out, nz, folded=frozenset()):
    ...
    for r in rows:
        ...
        if n in folded and not np.array_equal(ko, ki):
            ...
            continue          # dropped channels here are CONSTANTS, folded into the consumer's bias
