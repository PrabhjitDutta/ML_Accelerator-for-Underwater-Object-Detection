...
def main():
    ...
    # Map each Conv2d -> its parent ultralytics Conv (if any), so we can find sibling bn/act.
    conv2d_to_wrapper = {}
    ...
    for name, mod in model.named_modules():
        ...
        w = mod.weight.detach()  # [OC, IC/g, KH, KW]
        ...
        if wrapper is not None:
            ...
            # bare conv (Detect final 1x1): has its own bias, no BN, linear act
            save_bin(os.path.join(WEIGHTS, f"{name}.w.bin"), w.numpy())
    ...
    # plain-text manifest for the C++ loader: one line per conv, space-separated
    # name oc ic kh kw sh sw ph pw groups has_bn act(0=identity,1=silu)
