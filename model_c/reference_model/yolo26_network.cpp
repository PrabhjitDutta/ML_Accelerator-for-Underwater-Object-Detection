// The YOLO26s trunk graph, mirroring ultralytics' forward.
...
//   23 Detect pred-convs (reg_max=1, nc=4 -> 8 ch/scale) on layers [16,19,22].
#include <cstdio>
#include <cstdlib>
#include <future>
#include <stdexcept>
...
// YOLO26_HEADS_ONLY=1 dumps only the 6 head maps instead of every layer.
static bool heads_only() {
    // Read the value, so YOLO26_HEADS_ONLY=0 means off.
    static const bool v = [] {
...
// An independent branch: its own thread when the board host runs async (PS work overlaps another branch's
// kernel conv). Otherwise it runs at get().
...
// A residual Bottleneck's branch cv2(cv1(x)) alone; the consuming conv fuses the x + branch add into its quantizer.
...
static Tensor c3k(const Weights& W, const std::string& p, const Tensor& x) {
    Pre pb;                                                     // cv3's b, packed while the m chain runs
...
// C2f-style C3k2; mode 0 = shallow (Bottleneck m.0), 1 = deep (C3k m.0).
// pre: cv1's input already packed; cv2's t is packed while m runs.
...
static Tensor c2psa(const Weights& W, int i, const Tensor& x, const std::string& dir) {
    ...
    // l10_attn is a debug tap that recomputes the attention: only computed when full dumps are written.
...
static Tensor sppf(const Weights& W, int i, const Tensor& x) {
    ...
    // y1..y3 = 1..3 chained maxpools of y0, read only by cv2 (pooled on uint8 codes on the board)
    Tensor y = conv2d_cat({&y0, Src::pool(&y0, 1), Src::pool(&y0, 2), Src::pool(&y0, 3)}, W.get(s + ".cv2.conv"));
    // SPPF shortcut is always taken (c1 == c2); asserted so a width mismatch fails instead of dropping the residual.
...
std::vector<Tensor> run_trunk(const Weights& W, const Tensor& input, const std::string& dir) {
    ...
    // The neck's skip inputs (x6, x4, x13, x10) are packed into 13/16/19/22.cv1's X ahead of time.
    const Pre p13 = y26_board_prepack(&x6, W.get("13.cv1.conv"), true);
    Tensor x10 = c2psa(W, 10, x9, dir);           dump(dir, "10", x10);
    // Upsamples 11/14 are read only by 13.cv1/16.cv1: the board quantizes the half-size map and replicates codes.
    // They exist as tensors only for full dumps.
    ...
    for (int i = 0; i < 3; ++i) {
        // The one2many head (23.cv2/23.cv3) is training-only supervision; inference never reads it.
        // -DYOLO26_NO_O2M drops it. On by default because the export/ comparison scripts read the o2m_* dumps.
