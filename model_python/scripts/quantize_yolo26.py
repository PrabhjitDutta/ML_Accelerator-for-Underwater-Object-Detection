...
def main():
    ...
    def make_int_forward(mod, w_int, scale, sa_v):
        ...
        def fwd(x):
            xq = torch.clamp(torch.round(x / sa_v), -QMAX, QMAX)                            
            acc = F.conv2d(xq.double(), w_d, None, stride, padding, dilation, groups)               
            return acc.float() * sc + bias                                                         
        return fwd

    for name, mod in quant_convs:
        w = mod.weight.data
        sw = w.abs().amax(dim=(1, 2, 3)).clamp_min(1e-8) / QMAX                
        w_int = torch.clamp(torch.round(w / sw.view(-1, 1, 1, 1)), -QMAX, QMAX)
        scale = (sa[name] * sw).float()                                                     
